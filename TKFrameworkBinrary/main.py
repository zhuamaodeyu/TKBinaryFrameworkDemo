...
pool = multiprocessing.Pool(processes=multiprocessing.cpu_count()-1, maxtasksperchild=5)
result = []
...
def main(workspace_path, config_file_path, log_path):
    ...
    for dir in workspaces:
        ...
        if pod_file_list is None:
            ...
            # result.append(pool.apply_async(worker.worker, args=((dir, config, global_log_path, "debug"),)))
    pool.close()
    pool.join()
